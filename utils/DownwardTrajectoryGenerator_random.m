...
while ~valid
    ...
    y = RandomNumber(1)*sin(0.5*ttime)    +RandomNumber(0.5)*sin(1*ttime)   +RandomNumber(0.2)*sin(2*ttime)   +RandomNumber(0.1)*sin(3*ttime);

    roll =  randi([-15,15])*sin(ttime) + randi([-30,30])*sin(2*ttime); %0.8*sin(ttime); %sin(3*ttime)./10.*(sin(0.5*ttime)+1*cos(2*ttime)-0.3*sin(ttime));
    yaw =   randi([-15,15])*sin(ttime) + randi([-30,30])*sin(2*ttime);%zeros(n_timesteps,1); %pi/2*sin(2*ttime); %
    pitch = randi([-15,15])*sin(ttime) + randi([-30,30])*sin(2*ttime);%zeros(n_timesteps,1); %ones(n_timesteps,1)*15*pi/180; %zeros(n_timesteps,1); %cos(3*ttime)./10.*(sin(ttime)+0.5*cos(0.5*ttime)+0.05*sin(2*ttime));
